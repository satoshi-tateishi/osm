...
### Phase 17完了(測定ソースのSettingsをポップオーバー化)

`web/src/measurementList.ts`、(新規)`web/src/settingsPopover.ts`、`web/src/main.ts`、`web/src/style.css`

- 右ペインの常設Settings見出し・パネルを廃止し、Transfer Functionの各測定行のヘッダー右端へ歯車ボタンを追加した。測定一覧とGeneratorの表示領域を広く保ち、編集したい測定の設定だけを一時的に表示できるようにするためである。
- 歯車クリック時は`document.body`直下に1つだけ生成したポップオーバーへ既存のSettingsフォームとLevel/Ref/Peakメーターを描画する。同じ歯車の再クリック、外側クリック、Escape、閉じるボタンで閉じ、別の歯車ではDOMを増やさず同じポップオーバーの内容を切り替える。開いている対象Measurementが削除された場合も閉じる。
- ポップオーバーは歯車を基準に配置し、ビューポートの左右・上下端から8px以上離す。下側に収まらない場合は歯車の上へ反転し、非同期にSettingsフォームを描画した後にも実寸で位置を再計算する。高さは70vhまでとして内部スクロールさせ、Generatorの固定領域を維持する。
...
### Phase 19完了(Settingsポップオーバーに測定ソース削除ボタンを追加)

`web/src/settingsPanel.ts`、`web/src/main.ts`、`web/src/style.css`

- Transfer Function側からも測定ソースを削除できるよう、Settingsポップオーバーの末尾に赤い「削除」ボタンを追加した。他の設定操作との誤操作を防ぐため、Level/Ref/Peakメーターの下を境界線で区切り、通常アクションと視覚的に分離した。
