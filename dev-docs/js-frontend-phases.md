...
| Phase 19 | Settingsポップオーバーに測定ソース削除ボタンを追加 | 完了 |
...
## Phase 19: Settingsポップオーバーに測定ソース削除ボタンを追加

**目的**: 測定ソースを削除する導線が右ペインに存在しないため、Settingsポップオーバーの一番下に赤い「削除」ボタンを追加する。

**対象ファイル**: `web/src/settingsPanel.ts`(ボタンの追加・イベント配線)、`web/src/main.ts`(削除確認・実行の配線)、`web/src/style.css`(赤いボタンのスタイル)

**タスク**:
- [x] `renderSettingsPanel`のフォーム末尾(`#settings-meter`の後)に赤い「削除」ボタンを追加する
- [x] クリック時に確認ダイアログを出し、OKなら`sourceList.removeItem(uuid, true)`を呼んで削除する(Session Data左ペインの既存削除と同じ確認文言・パターンを踏襲)
- [x] 削除後はポップオーバーを閉じる
- [x] 動作確認・完了メモを本ファイルに追記

**依存Phase**: Phase 17完了後

**明示的にスコープ外とする項目**: 削除以外のフォーム項目・レイアウトの変更は行わない。

**完了メモ(2026-08-09)**: SettingsポップオーバーのLevel/Ref/Peakメーターの下に、危険操作として区切った赤い「削除」ボタンを追加した。Session Dataの既存削除と同じ確認文言を使い、確定時は開いているポップオーバーと選択状態を先に閉じてから、そのSettings payloadのuuidだけを`sourceList.removeItem(uuid, true)`で削除する。TypeScript/Viteの本番ビルドでコールバック型と配線を検証した。

**実機確認後の調整(2026-08-09)**: 高さ`70vh`では削除ボタンの表示にスクロールが必要だったため、ポップオーバーの上位置は維持しつつ、画面下端の安全余白まで使う`calc(100vh - 32px)`へ拡張した。低いウィンドウでフォームが収まらない場合の内部スクロールは維持する。

**実機確認(2026-08-09)**: 高さ調整後のポップオーバーで、スクロールせずに削除ボタンが表示されること、確認ダイアログで確定すると対象の測定ソースを正常に削除できることをmacOS実機で確認した。
